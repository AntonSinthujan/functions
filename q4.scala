object Q4 extends App {
    ...
    var accountList:List[Accounts] = List()
    ...
    val overdraft = (b:List[Accounts]) => b.filter(account => account.balance < 0.0)
    val totalBalance = (b:List[Accounts]) => b.foldLeft(0.0)((x, y) => x + y.balance)
    val interest = (b:List[Accounts]) => b.map(account => if(account.balance > 0) account.balance*0.05 else account.balance*0.1)
...
class Accounts(nic:String, val accId: Int, var balance: Double = 0.0){
